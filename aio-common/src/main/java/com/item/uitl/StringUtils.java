 ...
 ******************************************************************************/
package com.item.uitl;


import com.item.common.http.consts.HTTPConstant;
import com.item.common.http.enums.enums.HeaderNameEnum;
import com.item.common.http.enums.enums.HeaderValueEnum;
...
//            System.out.println(new String(bytes, offset, length));
    ...
    public static <T> ByteTree<T> scanByteTree(ByteBuffer buffer, ByteTree.EndMatcher endMatcher, ByteTree<T> cache) {
        ...
        while (position < limit && data[position] == HTTPConstant.SP) {
            position++;
        }
    ...
    private static void trimBuffer(ByteBuffer buffer) {
        while (buffer.hasRemaining() && buffer.get(buffer.position()) == HTTPConstant.SP) {
